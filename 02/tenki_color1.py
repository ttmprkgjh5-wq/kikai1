...
def show_final_outfit(items):
    ...
    window_title = ' vs '.join(labels)
    
    cv2.imshow(f'Final Outfit: {window_title}', combined_image)
...
if __name__ == "__main__":
    ...
    # --- 1. 設定とデータ読み込み ---
    CURRENT_TEMPERATURE = 7  # (例) 7度
    CLOSET_FILE = "closet.json"
    RULES_FILE = "rules.json"

    print(f"*** ファッションコーディネート提案 ***")
    print(f"現在の設定気温: {CURRENT_TEMPERATURE}度")
    ...
    # --- 2. 気温に基づく服の絞り込み ---
    allowed_types = get_allowed_clothing_types(CURRENT_TEMPERATURE, ALL_ITEMS_RULES)
    wearable_clothes = filter_my_closet(allowed_types, my_closet)

    if not wearable_clothes:
        print(f"\n[結果] 気温{CURRENT_TEMPERATURE}度で着られる服はありません。")
        sys.exit()

    # --- 3. 着用可能な服の代表色を抽出し、カテゴリ分類 ---
    print("\n--- 3. 着用可能な服の代表色を抽出中... ---")
    ...
    # ユーザーのコメント通り、気温に合うアウターがなければスキップ
    if not wearable_outers:
        print("\n--- 6. アウター検索 ---")
        print("  -> 気温に合う着用可能なアウターがありません。アウターなしを推奨します。")
    else:
        print(f"\n--- 6. ペアに合う最適なアウターを検索 ---")
        ...
        for outer in wearable_outers:
            ...
            # 総合スコア (例: 2つのスコアの平均)
            # (より厳しい判定なら min(score_with_top, score_with_bottom) もあり)
    ...
    # --- 7. 最終結果の表示 ---
    print("\n===================================")
    print(f"     気温 {CURRENT_TEMPERATURE}度 のおすすめコーデ")
    print("===================================")
