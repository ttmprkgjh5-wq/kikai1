...
train_dataset = datasets.MNIST(
    './data',               # データの保存先
    train = True,           # 学習用データを取得する（学習時に用いる画像と評価時に用いる画像を別の画像にするため）
...
test_dataset = datasets.MNIST(
    './data', 
    train = False,          # 評価用データを取得する（学習時に用いる画像と評価時に用いる画像を別の画像にするため）
    transform = transform
    )
...
for epoch in range(num_epochs): # 学習を繰り返し行う
    ...
    for inputs, labels in train_dataloader: # データローダーの中からデータとラベルをbatch_size個ずつ取り出している部分
